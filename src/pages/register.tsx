...
function Register() {
  ...
  const register = async () => {
    ...
    if (password !== re_password) error = 'Mật khẩu nhập lại sai'

    await userStore.Register(email, password, error)
      ...
      <div className="bg-white w-[800px] mx-auto rounded-lg">
        <div className="flex h-full">
          <div className="w-8/12 p-[44px] flex flex-wrap flex-col justify-center">
            ...
            <div className="form-control w-full mb-4">
              <div className="text-lg font-semibold mb-2">Nhập lại mật khẩu</div>
              <div className="border-[1px] rounded h-[48px]">
                <input className="h-full w-full p-4 rounded" type="password" value={re_password} onChange={(e) => setRepassword(e.target.value)}/>
              </div>
            </div>
            <div className="form-control w-full mb-4">
              <div className="border-[1px] rounded h-[48px] flex items-center justify-center">
                <button className="w-full h-full bg-[#FF424E] text-white rounded text-base" onClick={register}>Đăng ký</button>
              </div>
            </div>
            <div>
